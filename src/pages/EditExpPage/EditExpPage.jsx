...
//       if (exp.creator !== user?.id && exp.creator?.id !== user?.id) {
...
export default function EditExpPage({ user }) {
  ...
  // جلب تفاصيل التجربة بدون التحقق من الـ creator
  async function getAndSetDetail() {
    try {
      const exp = await expAPI.getById(id);
      console.log('Experience:', exp);  // تحقق من التجربة المرسلة من الـ API
      ...
      setFormData({
        title: exp.title,
        category: exp.category.id, // استخدم ID الفئة بدلاً من الكائن
        summary: exp.summary,
        image_path: exp.image_path,
      });
    } catch (err) {
  ...
  useEffect(() => {
    fetchCategories();
    getAndSetDetail();
  }, [id]);  // إعادة التحميل عند تغيير الـ id

  // تحديث البيانات المدخلة في النموذج
  function handleChange(e) {
    const { name, value } = e.target;
    setFormData((prevData) => ({ ...prevData, [name]: value }));
  }

  // إرسال التغييرات إلى الـ API
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      const updatedExp = await expAPI.update(formData, id);
